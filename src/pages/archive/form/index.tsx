...
const ArchiveFormPage = () => {
  ...
  useDidShow(() => {
    ...
    if (id) {
      ...
      if (archive && !archive.isDefault) {
        setArchiveId(archive.id)
        setNickname(archive.nickname)
        setGender(archive.gender)
        setCalendarType(archive.calendarType)
  ...
  const handleSubmit = () => {
    ...
    const newArchive: Archive = {
      id: archiveId || generateArchiveId(),
      ...
      isDefault: false,
      createdAt: archiveId ? (getArchiveById(archiveId)?.createdAt || Date.now()) : Date.now(),
      updatedAt: Date.now(),
    }
  ...
  return (
    <View className="min-h-full bg-white px-6 pt-4 pb-24">
      ...
      <View className="fixed left-0 right-0 bottom-0 bg-white border-t border-gray-100 px-6 py-4">
        <Button className="w-full bg-gray-900 text-white py-4 rounded-xl text-base font-medium" onClick={handleSubmit}>
          保存档案
        </Button>
      </View>
    </View>
