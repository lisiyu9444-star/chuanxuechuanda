...
2. 描述要围绕用神五行给出，例如用神为木可提生机勃勃、用神为金可提果断利落；description 总长度严格不超过50个汉字。
3. 只输出 JSON，不要任何额外文字。`
