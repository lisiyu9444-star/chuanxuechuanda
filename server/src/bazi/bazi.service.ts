...
【输出尺寸规格】
强制竖版 3:4 比例构图，顶部留白与底部留白比例为 1:2，确保画面重心稳定。画面中所有服装与配饰单品必须完整入镜，任何单品不得被画面边缘裁切、不得被其他单品遮挡。
...
【辅助单品 - 辅色=喜神】
配饰部分包含一只 ${accessoryColor} 的 ${items.bag} 和一双 ${accessoryColor} 的 ${items.shoes}，放置于服装右下方，不与服装重叠。

【点缀细节 - 点缀色】
${isFemale ? '首饰' : '配饰'}搭配包含${items.accessories}，采用${accessoryMaterial}作为视觉亮点，保证搭配美观且色彩呼应。

【摆放构图与光影】
采用上下三段式稳定构图：上衣/外套平铺于画面上方区域，下装平铺于画面中部区域，鞋包配饰整齐排列于画面下方区域。各单品之间保留充足间距、边界清晰、互不重叠，整体布局疏朗有序。
...
【模特要求】
- ${gender === 'female' ? `亚洲女性模特，${age || 25} 岁，身材匀称，五官精致自然` : `亚洲男性模特，${age || 25} 岁，身材挺拔，五官端正自然`}
- 必须展示完整面部，表情自然放松，妆容清淡精致，不可裁切面部
- 自然站立姿势，正面或微侧身
- 发型简洁，不遮挡服装细节与配饰
...
【内搭与得体性规范 - 必须严格遵守】
- 外套/西装/开衫/大衣内必须穿着可见的内搭，内搭领口和胸前区域必须清晰露出
- ${gender === 'female' ? '内搭可为衬衫、吊带背心、圆领/V领针织衫或T恤，颜色与外套协调且不过于接近' : '内搭必须为衬衫、圆领T恤或POLO衫，颜色与外套协调且不过于接近'}
- 整体穿搭必须端庄得体，禁止真空、低胸、透视、过度裸露肌肤或暴露胸部
- 服装必须合身整洁，不得出现撕裂、变形、穿模等异常状态
